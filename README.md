...
票票通 (票票通: 服务器后端版) (EasyTicket: BackEnd-Programe-Edition)   
是基于12306的快速抢票工具, 用爬虫和selenium等自动化技术调用系统浏览器实现。  

## 关于票票通后端版的简单介绍
与标准版票票通不同的是该服务器后端版是从标准版演化而来的, 考虑到各位买票有
提前买未开售的票和需要在手机端上使用该项目的需求, 决定开发此后端版票票通。  
(* 注: 本分支或是本版本由于刚刚建立, 距离可以实际使用请等候该版本的第一个稳定发行版
发行后再进行体验! 敬请期待!)
...
# 请注意, 由于该版本的票票通尚未完成开发，在执行此命令后安装的仍然是标准版票票通! 敬请期待!
```
